...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint_path", type=str, default="", help="Path to the checkpoint"
    )
    parser.add_argument(
        "--img_path",
        type=str,
        default="demo_images/universal/0000.png",
        help="Path to the image",
    )
    parser.add_argument(
        "--output_folder", type=str, default="outputs/", help="Output folder"
    )
    parser.add_argument(
        "--num_output_frames",
        type=int,
...
class InteractiveGameInference:
    ...
    def _init_models(self):
        ...
        pipeline = CausalInferencePipeline(
            self.config, generator=generator, vae_decoder=current_vae_decoder
        )
    ...
    def generate_videos(self):
        ...
        padding_video = torch.zeros_like(image).repeat(
            1, 1, 4 * (self.args.num_output_frames - 1), 1, 1
        )
        ...
        process_video(
            ...
            self.args.output_folder + "/demo.mp4",
            config,
            mouse_icon,
            mouse_scale=0.1,
            process_icon=False,
        ...
        process_video(
            video.astype(np.uint8),
            self.args.output_folder + "/demo_icon.mp4",
            config,
            mouse_icon,
            mouse_scale=0.1,
            process_icon=True,
