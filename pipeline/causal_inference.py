    ...
    if mode == "universal":
        ...
        while flag != 1:
            try:
                ...
                if (
                    idx_mouse in CAMERA_VALUE_MAP.keys()
                    and idx_keyboard in KEYBOARD_IDX.keys()
                ):
                    flag = 1
            except:
                pass
        ...
        while flag != 1:
            try:
                ...
                if len(idx_keyboard) == 0:
                    idx_keyboard += ["q"]
                assert idx_mouse in [["a"], ["d"], ["q"]] and idx_keyboard in [
                    ["q"],
                    ["w"],
                    ["s"],
                ]
    ...
    if replace != None:
        ...
        if mode != "templerun":
            conditional_dict["mouse_cond"][
                :, -last_frame_num + final_frame : final_frame
...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def inference(
        ...
        self.kv_cache1 = self.kv_cache_keyboard = self.kv_cache_mouse = (
    ...
    def _initialize_kv_cache_mouse_and_keyboard(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_cache_keyboard.append(
                {
                    "k": torch.zeros(
                        [batch_size, kv_cache_size, 16, 64], dtype=dtype, device=device
                    ),
                    "v": torch.zeros(
                        [batch_size, kv_cache_size, 16, 64], dtype=dtype, device=device
                    ),
            ...
            kv_cache_mouse.append(
                {
                    "k": torch.zeros(
                        [batch_size * self.frame_seq_length, kv_cache_size, 16, 64],
                        dtype=dtype,
                        device=device,
                    ),
                    "v": torch.zeros(
    ...
    def _initialize_crossattn_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            crossattn_cache.append(
                {
                    "k": torch.zeros(
                        [batch_size, 257, 12, 128], dtype=dtype, device=device
                    ),
                    "v": torch.zeros(
                        [batch_size, 257, 12, 128], dtype=dtype, device=device
                    ),
...
class CausalInferenceStreamingPipeline(torch.nn.Module):
    ...
    def inference(
        ...
        action_provider: Optional[
            Callable[[int], Optional[Dict[str, torch.Tensor]]]
        ] = None,
    ) -> torch.Tensor:
        """
        Perform inference on the given noise and text prompts.
        ...
        for current_num_frames in all_num_frames:
            ...
            if mode != "templerun":
                config = (
                    conditional_dict["keyboard_cond"][
                        0,
                        : 1 + 4 * (current_start_frame + self.num_frame_per_block - 1),
                    ]
                    .float()
                    .cpu()
                    .numpy(),
            ...
            process_video(
                ...
                output_folder + f"/{name}_current.mp4",
                config,
                mouse_icon,
                mouse_scale=0.1,
                process_icon=False,
        ...
        if mode != "templerun":
            config = (
                conditional_dict["keyboard_cond"][
                    0, : 1 + 4 * (current_start_frame + self.num_frame_per_block - 1)
                ]
                .float()
                .cpu()
                .numpy(),
        ...
        process_video(
            video.astype(np.uint8),
            output_folder + f"/{name}_icon.mp4",
            config,
            mouse_icon,
            mouse_scale=0.1,
            mode=mode,
    ...
    def generate_next_frames(
        ...
        if len(requested_frames) < num_frames:
            raise RuntimeError(
                f"Requested {num_frames} frames but only generated {len(requested_frames)}."
            )
